import { Box, Typography, List, ListItem, ListItemButton, Paper, alpha } from "@mui/material";
...
const ChannelItem = memo(({ name, id, active, canvasRef, onToggle }: ChannelItemProps) => (
  <ListItem disablePadding>
    ...
    >
      <Typography variant="caption" sx={{ fontWeight: "bold", color: active ? "primary.main" : "text.secondary" }}>
...
const ChannelsPanel = ({ imageData, channels, onToggleChannel, imgInfo }: ChannelsPanelProps) => {
  ...
  useEffect(() => {
    ...
    const drawChannel = (ref: React.RefObject<HTMLCanvasElement | null>, channelIndex: number) => {
      ...
      if (width > height) {
        ...
        thumbHeight = Math.round((height / width) * maxThumbSize);
      } else {
        thumbHeight = maxThumbSize;
        thumbWidth = Math.round((width / height) * maxThumbSize);
      }
    ...
    <Box sx={{ p: 2 }}>
      ...
      <List sx={{ pt: 0 }}>
        {isGrayscale ? (
          ...
          <>
            <ChannelItem name="Red" id="r" active={channels.r} canvasRef={rRef} onToggle={onToggleChannel} />
            <ChannelItem name="Green" id="g" active={channels.g} canvasRef={gRef} onToggle={onToggleChannel} />
            <ChannelItem name="Blue" id="b" active={channels.b} canvasRef={bRef} onToggle={onToggleChannel} />
          </>
        )}
        {hasAlpha && (
          <ChannelItem name="Alpha" id="a" active={channels.a} canvasRef={aRef} onToggle={onToggleChannel} />
        )}
      </List>
    </Box>
  );
